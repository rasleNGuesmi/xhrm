...
@Component({
  selector: 'app-dashboard-secretary',
  templateUrl: './dashboard-secretary.component.html',
  styleUrls: ['./dashboard-secretary.component.scss']
})
export class DashboardSecretaryComponent implements OnInit {
  ...
  // TODO DOING
  secretary_name: any = localStorage.getItem('username');
  remote: number;
  office: number;
  absent: number;
  total: number;
  ...
  onClick() {
    this.router.navigate(['/secretary/authorisation']);
  }
  ngOnInit(): void {
